...
// initialize deleting account state to false
var deletingAccount = false;

/**
 * Deletes a user's account and all of its data.
 * Password input is required.
 */
function deleteAccount() {
  // do not delete account if account is already being deleted
  if(deletingAccount) return;

  // record that account is being deleted
  ...
  // send api request to delete account
    ...
    .then(function (response) {
      // success response, go to home page
      if (response.status === 200) logout();
      // forbidden response
      else if (response.status === 403) {
        // display proper error message
        showMessage("incorrect password", false);
        // record that account is no longer being deleted
        deletingAccount = false;
      }
      else {
        // display error message
        showMessage("account deletion failed, please try again", false);
        // record that account is no longer being deleted
        deletingAccount = false;
      }
    })
...
/**
 * Consider pressing enter the same as clicking the delete account button.
 */
document.addEventListener('keypress', deleteAccountIfEnter);
function deleteAccountIfEnter(e) {
  if(e.keyCode == 13) deleteAccount();
}
